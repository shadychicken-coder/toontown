...
if (SERVER) then
		...
		if (client:canCraft()) then
			...
			if (blueprint) then
				if (blurprintNum == 1) then
					...
					for _, req in ipairs(blueprint.requirements) do
						...
						if (item < req[2]) then
							client:notifyLocalized("More Ingredients")
							return false
						else
					...
					for _, add in ipairs(blueprint.result) do
						for i=1, add[2] do
							...
							if (!succ) then
								if (res == "noSpace") then
									if (notified) then
										client:notifyLocalized("No Space")
					...
					if (blurprintNum > 1) then
						client:notifyLocalized("You can Craft Only One Bluprint")
					end
			...
			elseif (weapon) then
				if (weaponNum == 1) then
					...
					if (weaponNum > 1) then
						client:notifyLocalized("Craft Only One Weapon")
					end
				end
			else
				client:notifyLocalized("Nothing Craftable")
			end
		end
	end
	...
	function ENT:onDrawEntityInfo(alpha)
		...
		nut.util.drawText(L"Craft items and weapons", x, y + 16, ColorAlpha(color_white, alpha), 1, 1, "nutSmallFont", alpha * 0.65)
	end
end
